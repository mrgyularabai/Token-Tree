...
namespace Ozeki
{
    public class OzAIOpRMSNorm : OzAIOperation
    {
        public override OzAIOperationType Type => OzAIOperationType.RMS;
        ...
        public override bool IsPossible(out string error)
        {
            if (Epsilon == null)
            {
                error = $"{Type} is not possible, no epsilon provided";
                return false;
            }
            if (Part == null)
            {
                error = $"{Type} is not possible, no part provided";
                return false;
            }
